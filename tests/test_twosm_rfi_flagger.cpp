        ...
        int num_rfi_spikes)
{
    ...
    std::cout << "vis: " << visibilities[104] << "     " << predicted_flags[104] <<std::endl;
...
)
{
    ...
    sdp_Mem* antennas_in = sdp_mem_create_copy(antennas, antennas_location, status);
    ...
    sdp_twosm_algo_flagger(visibilities_in, thresholds_in, antennas_in, flags_in, status);
...
int main()
{
    ...
    }
    {
        ...
        run_and_check("Wrong flag location", false, false,
                      SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_DOUBLE, SDP_MEM_INT, SDP_MEM_INT,
                      SDP_MEM_CPU, SDP_MEM_CPU, SDP_MEM_GPU, SDP_MEM_CPU,
                      &status);
        assert(status == SDP_ERR_MEM_LOCATION);
    }
