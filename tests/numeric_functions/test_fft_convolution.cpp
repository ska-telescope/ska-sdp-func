    ...
    sdp_Error *status)
{
    ...
    // create test data
    sdp_Mem *in1 = sdp_mem_create(input_type, input_location, 2, in1_shape, status);
    sdp_mem_random_fill(in1, status);

    sdp_Mem *in2 = sdp_mem_create(input_type, input_location, 2, in2_shape, status);
    sdp_mem_random_fill(in2, status);

    // create output
    sdp_Mem *out = sdp_mem_create(output_type, output_location, 2, in1_shape, status);
...
int main()
{
    ...
    {
        sdp_Error status = SDP_SUCCESS;
        run_and_check("CPU, memory location mis-match", false,
                      SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_COMPLEX_DOUBLE,
