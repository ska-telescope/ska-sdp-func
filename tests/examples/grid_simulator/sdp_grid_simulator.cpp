...
int fits_write(sdp_Double2 *image, long imsize, long imsize_default, const char *filename){
	int status;
	...
	if(imsize == imsize_default){
		for (jj = 0; jj < naxes[1]; jj++)
			for (ii = 0; ii < naxes[0]; ii++){
				idx = jj*naxes[1] + ii;
				array[idx] = (float)(sqrt(image[idx].x*image[idx].x + image[idx].y*image[idx].y));
			}
	}
	else
	{
		int ratio = imsize/imsize_default;
		SDP_LOG_INFO("Compressing output image in %d times, %d -> %d", ratio, imsize, imsize_default);
			...
			for (ii = 0; ii < naxes[0]; ii++){
				...
				for(ll=llstart;ll<llstart+ratio;ll++)
					for(kk=kkstart;kk<kkstart+ratio;kk++){
						idx2 = ll*imsize + kk;
						array[idx] += (float)(sqrt(image[idx2].x*image[idx2].x + image[idx2].y*image[idx2].y));
					}
...
int main(int argc, char **argv) {
	...
	sdp_Mem* sources =
	            sdp_mem_create(sources_type, SDP_MEM_CPU, 2, sources_shape, &status);
	...
	sdp_mem_clear_contents(sources, &status);
    	...
    	for(size_t i=0;i<num_sources;i++)
    		SDP_LOG_INFO("%f %f %f", est_sources[3*i], est_sources[3*i+1], est_sources[3*i+2]);
	...
	sdp_Mem* grid_sim =
	            sdp_mem_create(grid_sim_type, SDP_MEM_CPU, 2, grid_sim_shape, &status);
	...
	sdp_mem_clear_contents(grid_sim, &status);
    	...
    	status_fits = fits_write(grid_out, grid_size, grid_size_default, filename_vis);
	...
	sdp_Mem* image_out =
	            sdp_mem_create(image_out_type, SDP_MEM_CPU, 2, image_out_shape, &status);
	...
	sdp_mem_clear_contents(image_out, &status);
    	...
    	status_fits = fits_write(image_fits, grid_size, grid_size_default, filename_img);
    	SDP_LOG_INFO("FITSIO status %d\n", status_fits);
