...
void xyz_to_uvw(const double xyz[][3],
		double ha,
		double dec,
		double *uvw_ha,
		int nants){
...
void xyz_to_baselines(const double vlas[][3],
			int nants_vla,
			double ha_start,
			double ha_end,
			double ha_step,
...
void simulate_point(double *dist_uvw, cufftDoubleComplex *vis_sim, int uvw_num, double l, double m, double amp){
		...
		for(i=0;i<uvw_num;++i){
			dot_product = dist_uvw[3*i]*l + dist_uvw[3*i+1]*m + dist_uvw[3*i+2]*n;
...
void sdp_grid_simulator_VLA(
	...
	for(i=0;i<num_sources;i++){
		//                                                   l                m                 Amp
		simulate_point(dist_uvw, vis_sim, uvw_num, est_sources[3*i+1], est_sources[3*i+2], est_sources[3*i]);
	}
	...
	for(i=0;i<uvw_num;++i){
		...
		if((iu_grid >= 0 && iu_grid < grid_size) && (iv_grid >= 0 && iv_grid < grid_size))
			{
			...
			SDP_LOG_INFO("Index is outside the range, %d %d %d", (int)i, (int)iu_grid, (int)iv_grid);
			j++;
		}
	}
	SDP_LOG_INFO("%d scaled vis points outside the range %d x %d", (int)j, (int)grid_size, (int)grid_size);
