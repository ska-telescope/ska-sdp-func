...
void sdp_cuFFTxT(
	...
	cufftDoubleComplex *grid_sim = (cufftDoubleComplex *)malloc(sizeof(cufftDoubleComplex) * grid_size * grid_size);
	cufftDoubleComplex *image_out = (cufftDoubleComplex *)malloc(sizeof(cufftDoubleComplex) * grid_size * grid_size);
    ...
    gpuAssert(cufftXtMemcpy(plan, dComplexSamples, grid_sim, CUFFT_COPY_HOST_TO_DEVICE));
