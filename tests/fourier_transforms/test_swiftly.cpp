...
void test_facet_to_subgrid_basic()
{
    ...
    sdp_Mem *prepared = sdp_mem_create(SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_CPU, 2, prepared_size, &status);
    int64_t contrib_size[] = { 1, xM_yN_size };
    sdp_Mem *contrib = sdp_mem_create(SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_CPU, 2, contrib_size, &status);
...
void test_facet_to_subgrid_dft()
{
    ...
    for (std::vector<int64_t> sources : source_pos_list) {
        ...
        SDP_LOG_INFO("test_facet_to_subgrid_dft: RMSE %g (%d samples)", rmse, count);
        assert(rmse < 8e-7);
    }
...
)
{
    ...
    sdp_Mem *contrib0 = sdp_mem_create(SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_CPU, 2, contrib0_size, &status);
    sdp_Mem *contrib0_t = sdp_mem_transpose(contrib0, 0, 1, &status);
    int64_t prepared1_size[] = { xM_yN_size, yN_size };
    sdp_Mem *prepared1 = sdp_mem_create(SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_CPU, 2, prepared1_size, &status);
    int64_t contrib1_size[] = { xM_yN_size, xM_yN_size };
    sdp_Mem *contrib1 = sdp_mem_create(SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_CPU, 2, contrib1_size, &status);
    sdp_Mem *contrib1_t = sdp_mem_transpose(contrib1, 0, 1, &status);
    int64_t subgrid0_size[] = { xM_size, xM_yN_size };
    sdp_Mem *subgrid0_image = sdp_mem_create(SDP_MEM_COMPLEX_DOUBLE, SDP_MEM_CPU, 2, subgrid0_size, &status);
    sdp_Mem *subgrid0_image_t = sdp_mem_transpose(subgrid0_image, 0, 1, &status);
...
void test_facet_to_subgrid_dft_2d()
{
    ...
    for (auto sources : source_pos_list) {
        ...
        for (facet_off1 = -20*facet_off_step;
            ...
            std::vector<std::pair<int64_t, int64_t> > new_sources(sources.size());
            for (isrc = 0; isrc < sources.size(); isrc++) {
                ...
                new_sources[isrc] = std::pair<int64_t, int64_t>(source0, source1);
            }
