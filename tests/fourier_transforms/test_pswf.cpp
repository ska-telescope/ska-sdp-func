...
template<> double val<float>(float x) { return x; }
template<> double val<double>(double x) { return x; }
template<> double val<std::complex<float> >(std::complex<float> x) { return x.real(); }
template<> double val<std::complex<double> >(std::complex<double> x) { return x.real(); }

template<typename num_t>
void check(const char *test_name, sdp_MemType typ,
           const int m, const double c, const int64_t size,
           const double *expected, const double limit)
{
...
void check_all(const char *test_name, 
               const int m, const double c, const int64_t size,
               const double *expected)
{
    std::string name = test_name;
    check<float>((name+" float").c_str(), SDP_MEM_FLOAT, m, c, size, expected, 6e-8);
    check<double>((name+" double").c_str(), SDP_MEM_DOUBLE, m, c, size, expected, 6e-13);
    check<std::complex<float> >((name+" complex float").c_str(), SDP_MEM_COMPLEX_FLOAT,
                                m, c, size, expected, 6e-8);
    check<std::complex<double> >((name+" complex double").c_str(), SDP_MEM_COMPLEX_DOUBLE,
                                 m, c, size, expected, 6e-13);
