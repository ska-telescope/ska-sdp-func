...
)
{
    ...
    for (i = 0; i < rows; i++)
    {
        for (j = 0; j < cols; j++)
        {
            padded_data[(i + pad_rows) * padded_cols +
                    (j + pad_cols)] = data[i * cols + j];
        }
    }
}
...
template<typename T>
inline void sdp_remove_padding_2d(
        complex<T>* padded_data,
        complex<T>* data,
        int64_t rows,
        int64_t cols,
...
)
{
    ...
    for (i = 0; i < original_rows; i++)
    {
        ...
        {
            data[i * original_cols +
                    j] =
                    padded_data[(i + (pad_rows - 1)) * cols +
                            (j + (pad_cols - 1))];
        }
    }
}
...
)
{
    ...
    sdp_Mem* in1_pad_mem = sdp_mem_create(data_type,
            SDP_MEM_CPU,
            2,
            pad_shape,
            status
    ...
    // pad in1
    sdp_pad_2d<T>(in1, in1_pad_ptr, in1_dim, in1_dim, extra_in1, extra_in1);

    // create memory to save padded input in2
    sdp_Mem* in2_pad_mem = sdp_mem_create(data_type,
            SDP_MEM_CPU,
            2,
            pad_shape,
            status
    ...
    sdp_Fft* in2_fft_plan = sdp_fft_create(in2_pad_mem,
            in2_pad_mem,
            2,
            1,
            status
    ...
    sdp_Mem* multiply_mem = sdp_mem_create(data_type,
            SDP_MEM_CPU,
            2,
            pad_shape,
            status
    ...
    sdp_fft_normalise<T>(multiply_ptr, pad_size);

    // shift the result to the center of the image
    sdp_fft_shift_2d<T>(multiply_ptr, pad_dim, pad_dim);

    // remove padding from the convolved result
    sdp_remove_padding_2d<T>(multiply_ptr,
            out,
            pad_dim,
            pad_dim,
            extra_in1,
...
)
{
    ...
    uint64_t num_blocks1[] = {
        (in1_dim + num_threads1[0] - 1) / num_threads1[0],
        (in1_dim + num_threads1[1] - 1) / num_threads1[1], 1
    };
    ...
    uint64_t num_blocks2[] = {
        (in2_dim + num_threads1[0] - 1) / num_threads1[0],
        (in2_dim + num_threads1[1] - 1) / num_threads1[1], 1
    };
    ...
    // shift the result to the center of the image
    sdp_Mem* shift_result_mem = sdp_mem_create(data_type,
            SDP_MEM_GPU,
            2,
            pad_shape,
            status
    ...
    uint64_t num_blocks5[] = {
        (half_rows + num_threads5[0] - 1) / num_threads5[0],
        (half_cols + num_threads5[1] - 1) / num_threads5[1], 1
    };
    ...
    uint64_t num_blocks6[] = {
        (in1_dim + num_threads6[0] - 1) / num_threads6[0],
        (in1_dim + num_threads6[1] - 1) / num_threads6[1], 1
    };
