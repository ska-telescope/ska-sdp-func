...
)
{
    ...
    if (i < half_rows && j < half_cols)
    {
        ...
        shifted_data[i * cols +
                j] = data[(i + half_rows) * cols + j + half_cols];
        // top right to bottom left
        shifted_data[(i + half_rows) * cols +
                j] = data[i * cols + j + half_cols];
        // top left to bottom right
        shifted_data[(i + half_rows) * cols + j +
                half_cols] = data[i * cols + j];
        // bottom left to top right
        shifted_data[i * cols + j +
                half_cols] = data[(i + half_rows) * cols + j];
    }
}
...
)
{
    ...
    if (i < original_rows && j < original_cols)
    {
        data[i * original_cols +
                j] =
                padded_data[(i + (pad_rows - 1)) * cols + (j + (pad_cols - 1))];
    }
}
