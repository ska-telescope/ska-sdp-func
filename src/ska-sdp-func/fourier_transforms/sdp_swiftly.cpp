    ...
    sdp_Error* status)
{
    ...
    const int64_t offs0 = mod_p(-xM_yN_size / 2 + xM_size/2 + fct_offs0, xM_size);
    const int64_t fct_offs1 = facet_offset1 / (image_size / xM_size);
    const int64_t offs1 = mod_p(-xM_yN_size / 2 + xM_size/2 + fct_offs1, xM_size);
    ...
    sdp_Error* status)
{
    ...
    for (i0 = 0; i0 < bc0_size; i0++) {
        for (i = 0; i < xM_size; i++) {
            sg(i0, i) = tmp[i0*xM_size + mod_p(i + subgrid_offset + xM_size/2, xM_size)];
        }
    }
}
    ...
    sdp_Error* status)
{
    ...
    for (i0 = 0; i0 < xM_size; i0++) {
        int64_t _i0 = mod_p(i0 + subgrid_offset0 + xM_size/2, xM_size)*xM_size;
    ...
    sdp_Error* status)
{
    ...
    for (; i0 < xM_yN_size; i0++) {
        ...
        for (; i1 < xM_yN_size; i1++) {
            contrib((i0 + fct_offs0 + xM_yN_size / 2) % xM_yN_size,
                    (i1 + fct_offs1 + xM_yN_size / 2) % xM_yN_size) =
                fn0 * Fn(i1) * sg_img(i0 + offs0 - xM_size, i1 + offs1 - xM_size);
        }
    }
    ...
    sdp_Error* status)
{
    ...
    // Parameter checks
    const int64_t yN_size = swiftly->yN_size;
    if (sdp_mem_num_dims(facet_out) > 1 && sdp_mem_shape_dim(facet_out, 1) > yN_size) {
        SDP_LOG_ERROR("Facet data too large (%d>%d)!", sdp_mem_shape_dim(facet_out, 1), yN_size);
        *status = SDP_ERR_INVALID_ARGUMENT;
    }
    ...
    for (i0 = 0; i0 < bc0_size; i0++) {
        ...
        if (start < end) {
            ...
            for (i = 0; i < end; i++) {
                fct(i0, i + yN_size - start) = pfct(i0, i) * Fb(i + yN_size + Fb_off);
            }
            for (i = start; i < yN_size; i++) {
                fct(i0, i - start) = pfct(i0, i) * Fb(i + Fb_off);
            }
