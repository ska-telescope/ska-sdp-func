...
)
{
    ...
    if (sdp_mem_shape_dim(antennas, 0) * (sdp_mem_shape_dim(antennas, 0) + 1) /
            2 != sdp_mem_shape_dim(vis, 1))
    {
...
)
{
    ...
    uint64_t num_elements = num_timesamples * num_baselines * num_channels *
            num_pols;
    ...
    {
        // selecting the id of an autocorrelation baseline
        // (an "antenna to itself" type of baseline) from the list of antennas
        uint64_t antenna_id = antennas[a];
        for (uint64_t c = 0; c < num_channels; c++)
        {
            for (uint64_t t = 1; t < num_timesamples; t++)
            {
                uint64_t pos_current = t * timesample_block + antenna_id *
                        baseline_block + c * channel_block; // current position
                uint64_t pos_minusone = (t - 1) * timesample_block +
                        antenna_id * baseline_block + c * channel_block; // position at t-1
                ...
                if (flags[pos_current] != 1)
                {
                    ...
                    // detecting uphill transition ('good' to 'bad')
                    // or remaining in the bad position
                    if (cnd0 || (((cnd2 && cnd4) || (cnd3 && cnd5)) && cnd6))
                    {
                        for (uint64_t base = 0; base < num_baselines; base++)
                        {
                            ...
                            {
                                uint64_t pos = t * timesample_block + base *
                                        baseline_block + c * channel_block + p;
                                flags[pos] = 1;
                            }
                        }
                    }
                    // downhill transition detection and back-tracking
                    // (if we detect a big downhill we know that we have been
                    // in the 'bad' state, so we flag the previous samples)
                    if (cnd1 && (cnd7 == 0))
                    {
                        uint64_t i = 0;
                        ...
                        {
                            for (uint64_t base = 0; base < num_baselines;
                                    base++)
                            {
                                for (uint64_t p = 0; p < num_pols; p++)
                                {
                                    uint64_t pos = t * timesample_block + base *
                                            baseline_block + c * channel_block +
                                            p;
                                    flags[pos] = 1;
                                }
                            }
                            i = i + 1;
                            pos = (t - i) * timesample_block + antenna_id *
                                    baseline_block + c * channel_block;
                        }
                    }
                }
        ...
        // we register the flag with number 2 in here to make calculating
        // the union of the flags in frequency and time directions easier.
        for (uint64_t t = 0; t < num_timesamples; t++)
        {
            for (uint64_t c = 1; c < num_channels; c++)
            {
                uint64_t pos_current = t * timesample_block + antenna_id *
                        baseline_block + c * channel_block;
                uint64_t pos_minusone = t * timesample_block + antenna_id *
                        baseline_block + (c - 1) * channel_block;
                double vis0 = std::abs(visibilities[pos_current]);
                double vis1 = std::abs(visibilities[pos_minusone]);
                dv_between_cur_one = vis0 - vis1;
                dv_ratio = dv_between_cur_one / vis1;
                if (flags[pos_current] != 1)
                {
                    ...
                    if (cnd1 && (cnd7 == 0))
                    {
                        ...
                        while ((flags[pos] == 0 || flags[pos] == 1) && t > i)
                        {
                            ...
                            i = i + 1;
                            pos = t * timesample_block + antenna_id *
                                    baseline_block + (c - i) * channel_block;
                        }
                    }
                }
            }
