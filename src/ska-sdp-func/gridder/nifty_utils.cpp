...
void CalculateSupportAndBeta(double upsampling, double epsilon, int &support, double &beta, int &status)
{
	...
	// default values
	status = 0;
    support = -1; // Full support size
	beta = -1.0;
	
	// calculate support from epsilon
    ...
    if (upsampling >= 2)
    {
		...
		if (support == -1)
		{
			fprintf(stderr, "Requested epsilon too small - minimum is 1e-13\n");
			status = -1000;
		}
	}
    else
	{
		fprintf(stderr, "Oversampling factor is too small\n");
		status = -1000;
    }

    // Calculate beta from support
	if ((support < 2) || (support > 15))
	{
		fprintf(stderr, "Unsupported support size of %i\n", support);
		status = -1000;
		return;
	}
    if (upsampling >= 2)
