 ...
 *
 * As the exponential of semicircle gridding kernel psi(u,v,w) is
 * separable, its Fourier transform Fourier(psi)(l,m,n) is likewise
 * separable into one-dimensional components C(l)C(m)C(n).
 * As psi is even and zero outside its support, the convolutional
 ...
 * Gauss-Legendre quadrature, as recommended in equation 3.10 of 'A Parallel
 * Non-uniform Fast Fourier Transform Library Based on an "Exponential of
 * Semicircle" Kernel' by Barnett, Magland, Klintenberg and only using
 * the p positive nodes):
 *   C(k) ~ Sum_{i=1}^{p} weight_i *
 *                   psi(supp * node_i) * cos(\pi * k * supp * node_i)
 *
    ...
    const FP* const __restrict__ quadrature_weights)
{
    ...
    for (uint32_t i = 0; i < p; i++)
        correction += quadrature_kernel[i] *
            cos(PI * k * support * quadrature_nodes[i]) * quadrature_weights[i];
 ...
 */
template<typename FP>
__device__ FP exp_semicircle(const FP beta, const FP x)
{
    const FP xx = x * x;
    return xx > FP(1) ? FP(0) : exp(beta * (sqrt(FP(1) - xx) - FP(1)));
}
...
/*
 * @brief 3D gridding/degridding kernel.
 *
 * Performs the gridding (or degridding) of visibilities across a subset
 * of w-planes.
 * Parallelised so each CUDA thread processes a single visibility.
 *
 * @param num_vis_rows  Number of rows in vis block (slowest dimension).
 * @param num_vis_chan  Number of channels in vis block (fastest dimension).
 * @param visibilities  INPUT(gridding) OR OUTPUT(degridding): Visibilities.
 * @param vis_weights   INPUT: Weight for each visibility.
 * @param uvw_coords    INPUT: (u, v, w) coordinates for each visibility.
 * @param freq_hz       INPUT: Array of frequencies per channel.
 * @param w_grid_stack  OUTPUT: Flat array containing 2D computed w-grids,
 *                      presumed initially clear.
 * @param grid_size     One dimensional size of w-plane
 *                      (image_size * upsampling), assumed square.
 * @param grid_start_w  Signed index of first w-grid in current subset stack.
 * @param num_w_grids_subset Number of w-grids bound in current subset stack.
 * @param support       Full support for gridding kernel
 * @param beta          Beta constant used in exponential of semicircle kernel.
 * @param uv_scale      Scaling factor for conversion of uv coords
 *                      to grid coordinates (grid_size * cell_size).
 * @param w_scale       Scaling factor for converting w-coord to
 *                      signed w-grid index.
 * @param min_plane_w   W-coordinate of smallest w-plane.
 * @param solving       Flag to use degridding instead of gridding.
 */
...
__global__ void sdp_cuda_nifty_gridder_gridding_3d(
    ...
    VFP2* __restrict__ visibilities,
    const VFP* const __restrict__ vis_weights,
    const FP3* const __restrict__ uvw_coords,
    const FP* const __restrict__ freq_hz,
    VFP2* __restrict__ w_grid_stack,
    const int grid_size,
    const int grid_start_w,
    const uint32_t num_w_grids_subset,
    const uint32_t support,
    const VFP beta,
    const FP uv_scale,
    const FP w_scale,
    const FP min_plane_w,
    const bool solving)
{
    ...
    if (i_chan >= num_vis_chan || i_row >= num_vis_rows) return;

    // Determine whether to flip visibility coordinates, so w is usually
    // positive.
    ...
    // NOTE confirm everyone's understanding of what support means
    // eg when even/odd.
    const FP half_support = FP(support) * FP(0.5);

    // Minimum and maximum coordinate on grid along u or v axes.
    const int grid_min_uv = -grid_size / 2;
    const int grid_max_uv = (grid_size - 1) / 2;

    // Calculate bounds of where gridding kernel will be applied for this
    // visibility.
    ...
    const int grid_w_max =
        min((int) floor(pos_w + half_support),
            grid_start_w + num_w_grids_subset - 1);
    if (grid_w_min > grid_w_max || grid_u_min > grid_u_max ||
    ...
    // Bound above the maximum possible support when precalculating kernel
    // values.
    VFP kernel_u[KERNEL_SUPPORT_BOUND], kernel_v[KERNEL_SUPPORT_BOUND];
    for (int grid_u = grid_u_min; grid_u <= grid_u_max; grid_u++)
    {
        kernel_u[grid_u - grid_u_min] =
            exp_semicircle(beta, (VFP) (grid_u - pos_u) * inv_half_support);
    }
    for (int grid_v = grid_v_min; grid_v <= grid_v_max; grid_v++)
    {
        kernel_v[grid_v - grid_v_min] =
            exp_semicircle(beta, (VFP) (grid_v - pos_v) * inv_half_support);
    }

    // Offset of origin along u or v axes.
    const int origin_offset_uv = grid_size / 2;

    // Iterate through each w-grid.
    for (int grid_w = grid_w_min; grid_w <= grid_w_max; grid_w++)
    {
        const VFP kernel_w =
            exp_semicircle(beta, (VFP) (grid_w - pos_w) * inv_half_support);
        const size_t grid_offset_w =
            (grid_w - grid_start_w) * size_t(grid_size * grid_size);
        ...
        for (int grid_u = grid_u_min; grid_u <= grid_u_max; grid_u++)
        {
            for (int grid_v = grid_v_min; grid_v <= grid_v_max; grid_v++)
            {
                ...
                {
                    vis_weighted.x +=
                        w_grid_stack[grid_offset_uvw].x * kernel_value;
                    vis_weighted.y +=
                        w_grid_stack[grid_offset_uvw].y * kernel_value;
                }
            }
        }
    }
...
/*
 * @brief 2D gridding/degridding kernel.
 *
 * Performs the gridding (or degridding) of visibilities across a subset
 * of w-planes.
 * Parallelised so each CUDA thread processes a single visibility.
...
)
{
    ...
    {
        visibilities[i_vis].x += vis_weighted.x;
        visibilities[i_vis].y += vis_weighted.y;
    }
}


/*
 * Applies the w screen phase shift and accumulation of each w layer
 * onto dirty image.
 * Parallelised so each CUDA thread processes one pixel in each quadrant
 * of the dirty image.
 *
 * @param dirty_image   INPUT & OUTPUT: real plane for accumulating
 *                      phase-corrected w-layers across batches.
 * @param image_size    One dimensional size of image plane
 *                      (grid_size / sigma), assumed square.
 * @param pixel_size    Converts pixel index (x, y) to normalised image
 *                      coordinate (l, m) where l, m between -0.5 and 0.5.
 * @param w_grid_stack  INPUT: flat array containing 2D computed w-layers
 *                      (w-layer = iFFT(w-grid)).
 * @param grid_size     One dimensional size of w-plane
 *                      (image_size * upsampling), assumed square.
 * @param grid_start_w  Index of first w-grid in current subset stack.
 * @param num_w_grids_subset Number of w-grids bound in current subset stack.
 * @param inv_w_scale   Inverse of scaling factor for converting w-coord to
 *                      signed w-grid index.
 * @param min_plane_w   W-coordinate of smallest w-plane.
 * @param perform_shift_fft Flag to (equivalently) rearrange each grid so
 *                          origin is at lower-left corner for FFT.
 * @param do_wstacking
 */
template<typename FP, typename FP2>
__global__ void apply_w_screen_and_sum(
    FP* __restrict__ dirty_image,
    const uint32_t image_size,
    const FP pixel_size,
    const FP2* const __restrict__ w_grid_stack,
    const uint32_t grid_size,
    const int grid_start_w,
    const uint32_t num_w_grids_subset,
    const FP inv_w_scale,
    const FP min_plane_w,
    const bool perform_shift_fft,
    const bool do_wstacking)
{
    ...
    // Allow extra in negative x and y directions, for asymmetric image centre.
    if (i <= (int) half_image_size && j <= (int) half_image_size)
    {
        ...
        // Offset of origin (in w layer) along l or m axes.
        const int origin_offset_grid_centre = (int) (grid_size / 2);
        const int grid_index_offset_image_centre =
            origin_offset_grid_centre * ((int) grid_size) +
            origin_offset_grid_centre;

        for (int grid_coord_w = grid_start_w;
             grid_coord_w < grid_start_w + (int) num_w_grids_subset;
             grid_coord_w++)
        {
            const FP l = pixel_size * (FP) i;
            const FP m = pixel_size * (FP) j;

            FP2 shift;
            if (do_wstacking)
            {
                const FP w = (FP) grid_coord_w * inv_w_scale + min_plane_w;
            ...
            const int grid_index_offset_w =
                (grid_coord_w - grid_start_w) * ((int) (grid_size * grid_size));
            const int grid_index_image_centre =
                grid_index_offset_w + grid_index_offset_image_centre;

            // Calculate the real component of the complex w layer value
            // multiplied by the complex phase shift Note w_grid_stack presumed
            // to be larger than dirty_image (sigma > 1) so has extra pixels
            // around boundary
            const FP2 w_layer_pos_pos = w_grid_stack
                [grid_index_image_centre + j * ((int) grid_size) + i];
            pixel_sum_pos_pos +=
                w_layer_pos_pos.x * shift.x - w_layer_pos_pos.y * shift.y;
            const FP2 w_layer_pos_neg = w_grid_stack
                [grid_index_image_centre - j * ((int) grid_size) + i];
            pixel_sum_pos_neg +=
                w_layer_pos_neg.x * shift.x - w_layer_pos_neg.y * shift.y;
            const FP2 w_layer_neg_pos = w_grid_stack
                [grid_index_image_centre + j * ((int) grid_size) - i];
            pixel_sum_neg_pos +=
                w_layer_neg_pos.x * shift.x - w_layer_neg_pos.y * shift.y;
            const FP2 w_layer_neg_neg = w_grid_stack
                [grid_index_image_centre - j * ((int) grid_size) - i];
            pixel_sum_neg_neg +=
                w_layer_neg_neg.x * shift.x - w_layer_neg_neg.y * shift.y;
        }

        // Equivalently rearrange each grid so origin is at lower-left corner
        // for FFT
        const bool odd_grid_coordinate = ((i + j) & 1) != 0;
        ...
        // Add the four pixel sums to the dirty image taking care to be within
        // bounds for positive x and y quadrants.
        // Offset of origin (in dirty image) along l or m axes.
        const int origin_offset_image_centre = (int) half_image_size;
        const int image_index_offset_image_centre =
            origin_offset_image_centre * ((int) image_size) +
            origin_offset_image_centre;

        // Special cases along centre or edges of image
        if (i < (int) half_image_size && j < (int) half_image_size)
        {
            dirty_image
                [image_index_offset_image_centre + j * ((int) image_size) +
                 i] += pixel_sum_pos_pos;
        }
 ...
 *
 * Parallelised so each CUDA thread processes one pixel in each quadrant of
 * the dirty image.
 *
 * @param dirty_image   INPUT: real plane for input dirty image.
 * @param image_size    One dimensional size of image plane
 *                      (grid_size / sigma), assumed square.
 * @param pixel_size    Converts pixel index (x, y) to normalised image
 *                      coordinate (l, m) where l, m between -0.5 and 0.5.
 * @param w_grid_stack  OUTPUT: flat array containing 2D computed w-layers
 *                      (w-layer = iFFT(w-grid)).
 * @param grid_size     One dimensional size of w-plane
 *                      (image_size * upsampling), assumed square.
 * @param grid_start_w  Index of first w-grid in current subset stack.
...
__global__ void reverse_w_screen_to_stack(
    const FP* const __restrict__ dirty_image,
    const uint32_t image_size,
    const FP pixel_size,
    FP2* __restrict__ w_grid_stack,
    const uint32_t grid_size,
    const int grid_start_w,
    const uint32_t num_w_grids_subset,
    const FP inv_w_scale,
    ...
    const bool do_wstacking)
{
    ...
    if (i <= (int) half_image_size && j <= (int) half_image_size)
    {
        // Obtain four pixels from the dirty image, taking care to be within
        // bounds for positive x and y quadrants.
        // Offset of origin (in dirty image) along l or m axes.
        const int origin_offset_image_centre = (int) half_image_size;
        const int image_index_offset_image_centre =
        ...
        if (j < (int) half_image_size && i < (int) half_image_size)
            dirty_image_pos_pos = dirty_image
                [image_index_offset_image_centre + j * ((int) image_size) + i];
        if (j < (int) half_image_size)
            dirty_image_neg_pos = dirty_image
                [image_index_offset_image_centre + j * ((int) image_size) - i];
        if (i < (int) half_image_size)
            dirty_image_pos_neg = dirty_image
                [image_index_offset_image_centre - j * ((int) image_size) + i];

        dirty_image_neg_neg = dirty_image
            [image_index_offset_image_centre - j * ((int) image_size) - i];
             ...
             grid_coord_w++)
        {
            ...
            if (do_wstacking)
            {
                ...
                shift = phase_shift<FP, FP2>(w, l, m, FP(1.0));
                // shift.y = -shift.y; // inverse of original phase shift
            ...
            // Calculate the complex product of the (real) dirty image by the
            // complex phase shift.
            // Special cases along centre or edges of image.
            FP2 out;
            if (i < (int) half_image_size && j < (int) half_image_size)
            {
                ...
                w_grid_stack
                    [grid_index_image_centre + j * ((int) grid_size) + i] = out;
            }
            if (j > 0 && i < (int) half_image_size)
            {
                out = shift;
    ...
    const bool do_wstacking)
{
    ...
    if (i <= (int) half_image_size && j <= (int) half_image_size)
    {
        const FP l = pixel_size * i;
        const FP m = pixel_size * j;
        const FP n = sqrt(FP(1.0) - l * l - m * m) - FP(1.0);
        const FP l_conv = conv_corr_kernel[i];
        const FP m_conv = conv_corr_kernel[j];
        ...
        // Note: scaling (everything after division) does not appear to be
        // present in reference NIFTY code so it may need to be removed if
        // testing this code against the reference code repo:
        // https://gitlab.mpcdf.mpg.de/ift/nifty_gridder
        FP correction = do_wstacking
            ? (l_conv * m_conv * n_conv)
            : (l_conv * m_conv * conv_corr_norm_factor * conv_corr_norm_factor);
