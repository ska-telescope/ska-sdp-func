        ...
        if (scales[scale_idx] == 0){
            ...
            for (int x = 0; x < length; x++) {
                for (int y = 0; y < length; y++) {
                    ...
                    scale_kern_list[i_list] = complex<T>(exp(-distance / two_sigma_square) / (M_PI * two_sigma_square), 0.0);
                }
            }
...
static void ms_clean_cornwell(
        ...
        sdp_Mem* psf_complex_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 2, psf_shape, status);
        ...
        sdp_Mem* cur_scaled_residual_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 2, dirty_img_shape, status);
        complex<T>* cur_scaled_residual_ptr = (complex<T>*)sdp_mem_data(cur_scaled_residual_mem);
        sdp_mem_clear_contents(cur_scaled_residual_mem, status);

        sdp_Mem* scaled_residuals_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 3, scaled_residuals_shape, status);
        complex<T>* scaled_residuals_ptr = (complex<T>*)sdp_mem_data(scaled_residuals_mem);
        sdp_mem_clear_contents(scaled_residuals_mem, status);

        sdp_Mem* scaled_psf_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 4, scaled_psf_shape, status);
        complex<T>* scaled_psf_ptr = (complex<T>*)sdp_mem_data(scaled_psf_mem);
        sdp_mem_clear_contents(scaled_psf_mem, status);

        sdp_Mem* cur_scaled_psf_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 2, psf_shape, status);
        complex<T>* cur_scaled_psf_ptr = (complex<T>*)sdp_mem_data(cur_scaled_psf_mem);
        sdp_mem_clear_contents(cur_scaled_psf_mem, status);

        sdp_Mem* cur_scaled_psf_pre_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 2, psf_shape, status);
        sdp_mem_clear_contents(cur_scaled_psf_pre_mem, status);

        sdp_Mem* scale_kern_list_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 3, scale_kern_list_shape, status);
        complex<T>* scale_kern_list_ptr = (complex<T>*)sdp_mem_data(scale_kern_list_mem);
        sdp_mem_clear_contents(scale_kern_list_mem, status);

        sdp_Mem* cur_1_scale_kern_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 2, psf_shape, status);
        complex<T>* cur_1_scale_kern_ptr = (complex<T>*)sdp_mem_data(cur_1_scale_kern_mem);
        sdp_mem_clear_contents(cur_1_scale_kern_mem, status);

        sdp_Mem* cur_2_scale_kern_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 2, psf_shape, status);
        complex<T>* cur_2_scale_kern_ptr = (complex<T>*)sdp_mem_data(cur_2_scale_kern_mem);
        sdp_mem_clear_contents(cur_2_scale_kern_mem, status);

        sdp_Mem* coupling_matrix_mem = sdp_mem_create(data_type, SDP_MEM_CPU, 2, coupling_matrix_shape, status);
        T* coupling_matrix_ptr = (T*)sdp_mem_data(coupling_matrix_mem);
        sdp_mem_clear_contents(coupling_matrix_mem, status);

        sdp_Mem* peak_per_scale_mem = sdp_mem_create(complex_data_type, SDP_MEM_CPU, 1, scale_list_shape, status);
        complex<T>* peak_per_scale_ptr = (complex<T>*)sdp_mem_data(peak_per_scale_mem);
        ...
        sdp_create_copy_complex<T>(residual_ptr, dirty_img_size, residual_complex_ptr);
        ...
        sdp_create_scale_kern<T>(scale_kern_list_ptr, scale_list_ptr, scale_dim, psf_dim);
        ...
        for (int s = 0; s < scale_dim; s++) {
            ...
            for (int x = 0; x < psf_dim; x++){
                for (int y = 0; y < psf_dim; y++){
                    ...
                    const unsigned int i_list_s = INDEX_3D(scale_dim,psf_dim,psf_dim,s,x,y);
            ...
            for (int p = 0; p < scale_dim; p++) {
                ...
                for (int x = 0; x < psf_dim; x++){
                    ...
                    const unsigned int i_list_p = INDEX_3D(scale_dim,psf_dim,psf_dim,p,x,y);
                    const unsigned int i_cur = INDEX_2D(psf_dim,psf_dim,x,y);
            ...
            sdp_fft_convolution(residual_complex_mem, cur_1_scale_kern_mem, cur_scaled_residual_mem, status);
        ...
        while (cur_cycle < cycle_limit) {
            ...
            const unsigned int i = INDEX_3D(scale_dim,dirty_img_dim,dirty_img_dim,max_scale,x_per_scale_ptr[max_scale],y_per_scale_ptr[max_scale]);
            if (std::real(scaled_residuals_ptr[i]) < threshold) {
                // stop = 1;
                SDP_LOG_DEBUG("msClean stopped at %f" , std::real(scaled_residuals_ptr[i]) );
            ...
            int64_t kern_x_start = 0, kern_x_end = 0, kern_y_start = 0, kern_y_end = 0;
            ...
            for (int x = kern_x_start, i = 0; x < kern_x_end; x++, i++){
                ...
                const unsigned int i_clean = INDEX_2D(dirty_img_dim,dirty_img_dim,i,j);
                    const unsigned int i_kern = INDEX_3D(scale_dim,psf_dim,psf_dim,max_scale,x,y);

                    clean_comp_complex_ptr[i_clean] += loop_gain * max_scaled_biased * scale_kern_list_ptr[i_kern];
            ...
            for (int s = 0; s < scale_dim; s++){
                for (int x = psf_x_start, i = 0; x < psf_x_end; x++, i++){
                    for (int y = psf_y_start, j = 0; y < psf_y_end; y++, j++){
                        ...
                        scaled_residuals_ptr[i_res] -= (complex<T>((loop_gain * max_scaled_biased),0) * scaled_psf_ptr[i_psf]);
        ...
        sdp_create_copy_real(clean_comp_complex_ptr, dirty_img_size, clean_comp_ptr);

        for (int x = 0; x < dirty_img_dim; x++){
            for (int y = 0; y < dirty_img_dim; y++){
                const unsigned int i_res = INDEX_3D(scale_dim,dirty_img_dim,dirty_img_dim,0,x,y);
                const unsigned int i_sky = INDEX_2D(dirty_img_dim,dirty_img_dim,x,y);

            skymodel_ptr[i_sky] = skymodel_ptr[i_sky] + std::real(scaled_residuals_ptr[i_res]);
            }
        }

        // release memory
        sdp_mem_ref_dec(psf_complex_mem);
        SDP_LOG_DEBUG("psf_complex_mem free");
        sdp_mem_ref_dec(clean_comp_complex_mem);
        SDP_LOG_DEBUG("clean_comp_complex_mem");
        sdp_mem_ref_dec(residual_complex_mem);
        SDP_LOG_DEBUG("residual_complex_mem");
        sdp_mem_ref_dec(cbeam_mem);
        SDP_LOG_DEBUG("cbeam_mem free");
        sdp_mem_ref_dec(cur_scaled_residual_mem);
        SDP_LOG_DEBUG("cur_scaled_residual_mem free");
        sdp_mem_ref_dec(scaled_residuals_mem);
        SDP_LOG_DEBUG("scaled_residuals_mem free");
        sdp_mem_ref_dec(scaled_psf_mem);
        SDP_LOG_DEBUG("scaled_psf_mem free");
        sdp_mem_ref_dec(cur_scaled_psf_mem);
        SDP_LOG_DEBUG("cur_scaled_psf_mem free");
        sdp_mem_ref_dec(cur_scaled_psf_pre_mem);
        SDP_LOG_DEBUG("cur_scaled_psf_pre_mem free");
        sdp_mem_ref_dec(scale_kern_list_mem);
        SDP_LOG_DEBUG("scale_kern_list_mem free");
        sdp_mem_ref_dec(cur_1_scale_kern_mem);
        SDP_LOG_DEBUG("cur_1_scale_kern_mem free");
        sdp_mem_ref_dec(cur_2_scale_kern_mem);
        SDP_LOG_DEBUG("cur_2_scale_kern_mem free");
        sdp_mem_ref_dec(coupling_matrix_mem);
        SDP_LOG_DEBUG("coupling_matrix_mem free");
        sdp_mem_ref_dec(peak_per_scale_mem);
        SDP_LOG_DEBUG("peak_per_scale_mem free");
        sdp_mem_ref_dec(index_per_scale_mem);
        SDP_LOG_DEBUG("index_per_scale_mem free");
        sdp_mem_ref_dec(x_per_scale_mem);
        SDP_LOG_DEBUG("x_per_scale_mem free");
        sdp_mem_ref_dec(y_per_scale_mem);
        SDP_LOG_DEBUG("y_per_scale_mem free");
        sdp_mem_ref_dec(convolution_result_mem);
        SDP_LOG_DEBUG("convolution_result_mem free");
    ...
    if (sdp_mem_location(dirty_img) == SDP_MEM_CPU)
    {
         if (data_type == SDP_MEM_DOUBLE){
            ...
            ms_clean_cornwell<float>(
                ...
                (const float*)sdp_mem_data_const(scale_list),
                (float)loop_gain,
                (float)threshold,
                cycle_limit,
                dirty_img_dim,
