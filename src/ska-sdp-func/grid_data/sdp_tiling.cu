...
__global__ void sdp_tile_count_wproj_gpu(
        ...
        const float w_scale,
        int* __restrict__ num_points_in_tiles,
        int* __restrict__ num_skipped
)
{
...
__global__ void sdp_bucket_sort_wproj_gpu(
        const int64_t num_w_planes,
        const int64_t* support,
        const int num_times,
        const int num_baselines,
...
SDP_CUDA_KERNEL(sdp_bucket_sort_simple_gpu<double, double, double, double, 4>)
