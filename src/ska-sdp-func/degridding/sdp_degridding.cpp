...

{
    ...
    if (sdp_mem_location(grid) != location ||
        sdp_mem_location(vis_coordinates) != location ||
    ...
    if (!sdp_mem_is_c_contiguous(grid) ||
        !sdp_mem_is_c_contiguous(vis_coordinates) ||
    ...
    if (sdp_mem_type(grid) != SDP_MEM_COMPLEX_DOUBLE||
        sdp_mem_type(vis_coordinates) != SDP_MEM_DOUBLE||
        sdp_mem_type(uv_kernel) != SDP_MEM_DOUBLE||
        sdp_mem_type(w_kernel) != SDP_MEM_FLOAT||
