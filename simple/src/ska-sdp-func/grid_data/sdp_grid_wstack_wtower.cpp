...
struct sdp_SubgridTask {
        ...
        sdp_SubgridTask()
            : iu(0),
              iv(0),
              num_chunks(0),
              num_rows_per_chunk(0),
              num_rows_total(0),
              num_vis(0),
              row_start(0),
              row_end(0) {}

        sdp_SubgridTask(int64_t iu, int64_t iv, int64_t num_vis,
                        int64_t num_rows, int64_t num_vis_per_chunk)
            : iu(iu),
              iv(iv),
              num_chunks(0),
              num_rows_per_chunk(0),
            ...
            if (num_chunks > 0) {
                num_rows_per_chunk =
                    (num_rows_total + num_chunks - 1) / num_chunks;
            }
        }
};

// Count visibilities in each sub-grid, create and return a list of tasks.
vector<sdp_SubgridTask>
count_visibilities(const sdp_Mem *uvw, const sdp_Mem *vis, const sdp_Mem *start_ch_w, const sdp_Mem *end_ch_w, 
                   vector<sdp_Mem *> &start_ch_uv, vector<sdp_Mem *> &end_ch_uv, double freq0_hz, double dfreq_hz, 
                   double eff_sg_dist,int64_t min_iu, int64_t max_iu, int64_t min_iv, int64_t max_iv, int64_t iw, 
                   int verbosity, sdp_Timers *SDP_TMR_HANDLE, sdp_Error *status) {
    ...
    const int64_t num_vis_per_chunk = ((num_rows * num_chan) / (int64_t)num_threads);
...
#pragma omp parallel for num_threads(num_threads) collapse(2)
    for (int64_t iu = min_iu; iu <= max_iu; ++iu) {
        for (int64_t iv = min_iv; iv <= max_iv; ++iv) {
            ...
            sdp_gridder_clamp_channels_uv(uvw, freq0_hz, dfreq_hz, start_ch_w, end_ch_w, min_u, max_u, 
                                          min_v, max_v, start_ch_uv[tid], end_ch_uv[tid], -1, -1, status);

            sdp_gridder_sum_diff(end_ch_uv[tid], start_ch_uv[tid], &num_vis, -1, -1, status);

            int64_t subgrid = (iu - min_iu) * num_subgrids_v + iv - min_iv;
            tasks[subgrid] = sdp_SubgridTask(iu, iv, num_vis, num_rows, num_vis_per_chunk);

            if (verbosity > 1 && num_vis > 0) {
                sdp_mutex_lock(mutex);
                SDP_LOG_INFO("subgrid %d/%d/%d: " "%" PRId64 " visibilities in %" PRId64 " chunks", 
...
void report_timing(const vector<sdp_GridderWtowerUVW *> kernel, int verbosity, int gridding, 
                   int num_w_planes, int num_subgrids_u, int num_subgrids_v, double w_tower_height, 
                   const sdp_Mem *vis, const sdp_Timers *SDP_TMR_HANDLE, sdp_Error *status) {
    if (*status || verbosity == 0) return;

    int total_w_planes = 0;
    for (size_t i = 0; i < kernel.size(); ++i) {
        total_w_planes += sdp_gridder_wtower_uvw_num_w_planes(kernel[i], gridding);
    }
    ...
    SDP_LOG_INFO("| Total w-layers processed  : %d (in gridder kernels)", total_w_planes);
    SDP_LOG_INFO("| Number of sub-grids (u, v): (%d, %d)", num_subgrids_u, num_subgrids_v);
    SDP_LOG_INFO("| Image size (pixels)       : (%d, %d)", image_size, image_size);
    SDP_LOG_INFO("| Sub-grid size (pixels)    : (%d, %d)", subgrid_size, subgrid_size);
    SDP_LOG_INFO("| Vis shape (rows, chans)   : (%d, %d)", sdp_mem_shape_dim(vis, 0), sdp_mem_shape_dim(vis, 1));
...
void sdp_grid_wstack_wtower_degrid_all( const sdp_Mem *image, double freq0_hz, double dfreq_hz, const sdp_Mem *uvw, 
                                       int subgrid_size, double theta, double w_step, double shear_u, double shear_v, 
                                       int support, int oversampling, int w_support, int w_oversampling, double subgrid_frac, 
                                       double w_tower_height, int verbosity, sdp_Mem *vis, int num_threads, sdp_Error *status) {
    if (*status) return;
    ...
    // Set up timers.
    SDP_TMR_CREATE("Degridding", loc == SDP_MEM_CPU ? SDP_TIMER_NATIVE : SDP_TIMER_CUDA, num_threads);
...
#pragma omp parallel for num_threads(num_threads)
    for (int i = 0; i < num_threads; ++i) {
        kernel[i] = sdp_gridder_wtower_uvw_create(image_size, subgrid_size, theta, w_step, shear_u, shear_v, 
                                                  support, oversampling, w_support, w_oversampling, status);
        subgrid[i] = sdp_mem_create(sdp_mem_type(vis), loc, 2, subgrid_shape, status);
        ifft_subgrid[i] = sdp_fft_create(subgrid[i], subgrid[i], 2, false, status);
    ...
    sdp_gridder_uvw_bounds_all(uvw, freq0_hz, dfreq_hz, start_ch, end_ch, uvw_min, uvw_max, status);
    ...
    sdp_Mem *grid =sdp_mem_create(sdp_mem_type(vis), loc, 2, grid_shape, status);
    ...
    for (int64_t iw = min_iw; iw <= max_iw; ++iw) {
        ...
        sdp_gridder_clamp_channels_single(uvw, 2, freq0_hz, dfreq_hz, start_ch, end_ch, min_w, max_w, 
                                          start_ch_w, end_ch_w, -1, -1, status);
        int64_t num_vis = 0;
        ...
        sdp_gridder_wtower_uvw_degrid_correct(kernel[0], grid, 0, 0, iw * w_tower_height, status);
        ...
        vector<sdp_SubgridTask> tasks = count_visibilities(uvw, vis, start_ch_w, end_ch_w, start_ch_uv, end_ch_uv, 
                                                           freq0_hz, dfreq_hz, eff_sg_dist, min_iu, max_iu, 
                                                           min_iv, max_iv, iw, verbosity, SDP_TMR_HANDLE, status);
...
#pragma omp parallel num_threads(num_threads)
        {
            for (;;) {
                ...
                sdp_gridder_clamp_channels_uv(uvw, freq0_hz, dfreq_hz, start_ch_w, end_ch_w, min_u, max_u, min_v, max_v, 
                                              start_ch_uv[tid], end_ch_uv[tid], start_row, end_row, status);
                sdp_gridder_sum_diff(end_ch_uv[tid], start_ch_uv[tid], &num_vis, start_row, end_row, status);
...
#pragma omp atomic
                ...
                sdp_gridder_subgrid_cut_out(grid, iu * eff_sg_size, iv * eff_sg_size, subgrid[tid], status);
                sdp_fft_exec_shift(ifft_subgrid[tid], subgrid[tid], 1, status);

                // Degrid visibilities from sub-grid.
                node = sdp_timers_push(SDP_TMR_HANDLE, "Process sub-grid stack", tid, node);
                sdp_gridder_wtower_uvw_degrid(kernel[tid], subgrid[tid], iu * eff_sg_size, iv * eff_sg_size, iw * w_tower_height, 
                                              freq0_hz, dfreq_hz, uvw, start_ch_uv[tid], end_ch_uv[tid], vis, start_row, end_row, status);
        ...
        if (vis_count_check != num_vis) {
            SDP_LOG_CRITICAL("Processed %d but expected %d visibilities", (int)vis_count_check, (int)num_vis);
            exit(1);
        }
    }

    // Report timing.
    report_timing(kernel, verbosity, 0, num_w_planes, num_subgrids_u, num_subgrids_v, w_tower_height, vis, SDP_TMR_HANDLE, status);
...
void sdp_grid_wstack_wtower_grid_all(const sdp_Mem *vis, double freq0_hz, double dfreq_hz, const sdp_Mem *uvw, 
                                     int subgrid_size, double theta, double w_step, double shear_u, double shear_v, 
                                     int support, int oversampling, int w_support, int w_oversampling, double subgrid_frac, 
                                     double w_tower_height, int verbosity, sdp_Mem *image, int num_threads, sdp_Error *status) {
    if (*status) return;
    ...
    // Set up timers.
    SDP_TMR_CREATE("Gridding", loc == SDP_MEM_CPU ? SDP_TIMER_NATIVE : SDP_TIMER_CUDA, num_threads);
    SDP_TMR_CREATE_SET("Process sub-grid stack", num_threads);
...
#pragma omp parallel for num_threads(num_threads)
    for (int i = 0; i < num_threads; ++i) {
        kernel[i] = sdp_gridder_wtower_uvw_create(image_size, subgrid_size, theta, w_step, shear_u, shear_v, 
                                                  support, oversampling, w_support, w_oversampling, status);
        subgrid[i] = sdp_mem_create(sdp_mem_type(vis), loc, 2, subgrid_shape, status);
        fft_subgrid[i] = sdp_fft_create(subgrid[i], subgrid[i], 2, true, status);
        start_ch_uv[i] = sdp_mem_create(SDP_MEM_INT, loc, 1, &num_rows, status);
        end_ch_uv[i] = sdp_mem_create(SDP_MEM_INT, loc, 1, &num_rows, status);
    }
    ...
    for (int64_t iw = min_iw; iw <= max_iw; ++iw) {
        ...
        double min_w = iw * w_stack_dist - w_stack_dist / 2;
        double max_w = (iw + 1) * w_stack_dist - w_stack_dist / 2;
        sdp_gridder_clamp_channels_single(uvw, 2, freq0_hz, dfreq_hz, start_ch, end_ch, min_w, max_w, start_ch_w, end_ch_w, -1, -1, status);
        int64_t num_vis = 0;
        sdp_gridder_sum_diff(end_ch_w, start_ch_w, &num_vis, -1, -1, status);
...
#pragma omp atomic
                ...
                sdp_gridder_wtower_uvw_grid(kernel[tid], vis, uvw, start_ch_uv[tid], end_ch_uv[tid], freq0_hz, dfreq_hz, 
                                            subgrid[tid], iu * eff_sg_size, iv * eff_sg_size, iw * w_tower_height, 
...
#pragma omp critical(subgrid_add)
                sdp_gridder_subgrid_add(grid, -iu * eff_sg_size, -iv * eff_sg_size, subgrid[tid], sg_factor, status);
            }
        }
        sdp_mutex_free(mutex);
        if (vis_count_check != num_vis) {
        ...
        sdp_gridder_wtower_uvw_grid_correct(kernel[0], grid, 0, 0, iw * w_tower_height, status);
    ...
    // Report timing.
    report_timing(kernel, verbosity, 1, num_w_planes, num_subgrids_u, num_subgrids_v, w_tower_height, vis, SDP_TMR_HANDLE, status);
